...
Alletime = '';
//获取用户信息
function getinfo(type,star){
	var u = '/index.php/center/taskverify/getregtask';
	stime = Allstime;
	etime = Alletime;
	var d = 'type='+type+'&star='+star+'&stime='+stime+'&etime='+etime;
	var f = function(res){
		...
		if (response['status']==request_succ) {
			...
			switch(response['data']['type']){
				case '3':
				    isto = '审核通过';
				    $('.digitalorder').html('审核通过');
				    break;
				case '6':
				    isto = '<span class="passed adopted">通过</span><div class="line"></div><span class="passed refer">未通过</span>';
				    $('.digitalorder').html('未审核');
				    break;
				case '7':
				    isto = '<span class="passed adopted">通过</span><div class="line"></div><span class="passed refer">未通过</span>';
				    $('.digitalorder').html('审核未通过');
				    break;
			}
			var list = '';
		    $.each(response['data']['re'], function(i, v) {
		    	list+='<div class="group">'
                            +'<div class="graph">'
                                +'<div class="case"></div>'
                            +'</div>'
                            +'<div class="tel">'+v['wx_mobile']+'</div>'
                            +'<div class="times">'+formatDate(v['log_content'])+'</div>'
                            +'<div class="state" opendid="'+v['wx_openid']+'" lid="'+v['log_id']+'">'
                            +isto
                            +'</div>'
                        +'</div>';
		    });
            ...
            Allstar = star;
            Alltype = type;
            page(10,Allstar,response['data']['num']);
	    }else{
            $('.contents').html('');
	    }
	}
...
//获取时间获取信息
function timegetinfo(a){
	if (a==1) {
		var myDate = new Date();
	    Allstime = myDate.getFullYear()+'-'+(myDate.getMonth()+1)+'-'+myDate.getDate();
	    myDate.setDate(myDate.getDate()+1);
	    Alletime = myDate.getFullYear()+'-'+(myDate.getMonth()+1)+'-'+myDate.getDate();
	}else if (a==2) {
		var myDate = new Date();
	    Alletime = myDate.getFullYear()+'-'+(myDate.getMonth()+1)+'-'+myDate.getDate();
	    myDate.setDate(myDate.getDate()-1);
...
function getcheck(){
	...
	for (var i = 0; i < nok.length; i++) {
		nocheckok += nok.eq(i).parent().attr('lid')+',';
	};
	if (checkok==''&&nocheckok=='') {return ;};
    var openids=openids.substring(0,openids.length-1);
    var checkok=checkok.substring(0,checkok.length-1);
    var nocheckok=nocheckok.substring(0,nocheckok.length-1);
	var u = '/index.php/center/taskverify/uptaskinfo';
	var d = 'data='+checkok+'&nodata='+nocheckok+'&openid='+openids;
	var f = function(res){
		var response = eval(res);
		if (response['status']==request_succ) {
			alert(response['msg']);
			UrlGoto('/view/control/dataAudit.html');
		}
	}
	AjaxRequest(u,d,f);
}
