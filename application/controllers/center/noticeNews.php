 ...
 *
 */
class  NoticeNews  extends  CI_Controller{
    ...
    function getlables(){ 
        ...
        $this->load->model('center/notice_model');
        $result = $this->notice_model->getalllabel();
        if ($result === false) {
    ...
    function checkpost(){
        ...
        $info['akey'] = Universal::filter($this->input->post('akey',true));
    ...
    function putextinfo(){
        ...
        $info = $this->checkpost();
        $this->load->model('center/notice_model');
        $result = $this->notice_model->addarticle($info);
        if ($result == false) {
            Universal::Output($this->config->item('request_fall'),'',$this->notice_model->msg,'');
        }
        Universal::Output($this->config->item('request_succ'),'','',$result);
    }
    /**
    ...
    function getarticlelist(){
        ...
        $this->load->model('center/notice_model');
        $result['articles'] = $this->notice_model->getarticles($num);
        $result['num'] = $this->notice_model->getnums();
        Universal::Output($this->config->item('request_succ'),'','',$result);
    }
    /**
     * 获取文章列表
    ...
    function getarticle(){
        ...
        $this->load->model('center/notice_model');
        $result = $this->notice_model->gethisarticle($id);
        if ($result===false) {
            Universal::Output($this->config->item('request_fall'),'未取得相关信息','','');
        }
        $result['article']['0']['text'] = json_decode($result['article']['0']['text']);
        Universal::Output($this->config->item('request_succ'),'','',$result);
    }
    /**
     * 更新文章
    ...
    function uparticle(){
        ...
        $this->load->model('center/notice_model');
        $result = $this->notice_model->uparticleinfo($info,$this->input->post('id',true));
        if ($result===false) {
            Universal::Output($this->config->item('request_fall'),$this->notice_model->msg,'','');
        }
