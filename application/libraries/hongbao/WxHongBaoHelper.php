...
class WxHongBaoHelper{
    var $partnerkey='beijingmaijinkejiwangkun20150521';
	...
	function check_transferssign_parameters(){
	...
	protected  function get_sign_transfers(){
	    try {
	        if (null == $this->partnerkey || "" == $this->partnerkey ) {
	            throw new SDKRuntimeException("密钥不能为空！" . "<br>");
	        }
	        if($this->check_transferssign_parameters() == false) {   //检查生成签名参数
			   throw new SDKRuntimeException("生成签名参数缺失！" . "<br>");
	        }
	        $commonUtil = new CommonUtil();
	        ksort($this->parameters);
	...
	*/
	 ...
	 * 生成转账 请求数据 xml
	 */
    function create_transfers_xml($retcode = 0, $reterrmsg = "ok"){
		 try {
		    $this->setParameter('sign', $this->get_sign_transfers());
		    $commonUtil = new CommonUtil();
		    return  $commonUtil->arrayToXml($this->parameters);
