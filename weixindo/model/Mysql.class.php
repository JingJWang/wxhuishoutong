...
class MySQL{
	...
	public function __construct($connect = true, $database = null, $server = null, $username = null, $password = null, $charset = null) {
		global $data,$host,$user,$pass,$dbcharset; //2013-11 mas�޸�
		if ($database == null) $this->db_dbname  = 'wxmj';
		if ($server   == null) $this->db_host    = 'localhost';
		if ($username == null) $this->db_user    = 'wxmj';
		if ($password == null) $this->db_pass    = 'yuruhao@2015';
	...
	*/
	...
	static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT) {
		...
		switch (strtolower(trim($datatype))) {
			...
			case "char":
				if (strlen($value) == 0) {
					$return_value = "''"; //$return_value = "NULL"; 2013-11-18 mas�޸�
				} else {
...
class DB_MySql
{
	...
	{
		global $host,$user,$pass,$data,$dbcharset;
	...
	public function dbquery($cmd,&$res,$bfetch = true)
	{
		if($bfetch == true)
		{
			$tmp = mysql_query($cmd,$this->dbh);
			if(!$tmp) return false;
